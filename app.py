...
def ensure_player_shape(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in data.get("edition_effects", []) or []:
        effect_keys = BATTER_KEYS if player_type == "野手" else PITCHER_KEYS
        effect = {k: to_int((item.get("effect") or {}).get(k)) for k in effect_keys}
...
- base は必ず画像内の「合計」行から読み取ってください。
- スキル画像が一緒にある場合でも、base を 0 にしないでください。
- 「合計」行が見える画像がある場合、base の各能力は必ず数値を入れてください。
- スキルやエディション効果は base とは別に入れてください。
